...
let records = [
  [Mp_data.Longlongint (Big_int.big_int_of_int 1); 
   ...
   Mp_data.Binary (Buffer.add_string binary1 "binary : ààéé\"\"çç''__((èè--èè&&ôö$£µ*ù1234567890auebsqtxbneu<>?,;./:§!}]@^`|[{#~¹azerty¤====+-*/.'\"abcdef"; binary1);
  ...
  [Mp_data.Longlongint (Big_int.big_int_of_int 2); 
   ...
   Mp_data.Null;
   (* binary value has right padding with 0x00 so avoid the problem by putting exactly the same number of characters than the column size *)
   Mp_data.Binary (Buffer.add_string binary2 "binary : ààéé\"\"çç''__((èè--èè&&ôö$£µ*ù1234567890auebsqtxbneu<>?,;./:§!}]@^`|[{#~¹azerty¤====+-*/.'\"abcdef"; binary2);
...
let records_bigstring = [
  [Mp_data.String (latin1_to_ut8(Fixture_config.build_string 255)); Mp_data.Binary (Buffer.add_string bigbinarybuffer1 (Fixture_config.build_string 255); bigbinarybuffer1);];
  [Mp_data.String (latin1_to_ut8(Fixture_config.build_string 255)); Mp_data.Binary (Buffer.add_string bigbinarybuffer2 (Fixture_config.build_string 255); bigbinarybuffer2);];
];;

let records_bigvarchar = [
  [Mp_data.Varstring (latin1_to_ut8(Fixture_config.build_string 21844))];
  [Mp_data.Varstring (latin1_to_ut8(Fixture_config.build_string 21844))];
];;
