...
let build_ok_insert id = 
  { Mp_client.affected_rows = Int64.one;
    Mp_client.insert_id = Int64.of_int id;
...
let test1 connection testfile2 testfile3 insert_var_string = 
  ...
  let () = 
    ...
    let params = [Mp_data.Varstring ""] in
    assert_equal ~msg:sql 
      (build_ok_insert 8)
      (Test_query.try_query ~f:(Mp_client.get_result_ok(Mp_client.execute ~connection:connection ~statement:p ~params:params ())) ~sql:sql)
  in
  ()
;;
