...
let test1 connection records = 
  let () = 
    ...
    assert_equal ~msg:sql 
	 ...
	 ~f:( (Mp_client.get_result_set(Mp_client.execute ~connection:connection ~statement:p ())).Mp_result_set_packet.rows ) 
	 ~sql:sql)
  in
  let () = 
  ...
  let () = 
    ...
    let stmt = Mp_client.create_statement_from_string sql in
    let p = Mp_client.prepare ~connection:connection ~statement:stmt in
    let params = [Mp_data.Null] in
    assert_equal ~msg:sql 
      ~cmp:records_equals
      ( (fields, []) )
      (Test_query.try_query 
	 ~f:( (Mp_client.get_result_set(Mp_client.execute ~connection:connection ~statement:p ~params:params ())).Mp_result_set_packet.rows ) 
	 ~sql:sql)
  in
  ...
  let () = 
	...
	( (fields, [List.nth records 1]) )
	(Test_query.try_query 
	 ~f:( (Mp_client.get_result_set(Mp_client.execute ~connection:connection ~statement:p ~params:params ~bind:Mp_execute.No_bind ())).Mp_result_set_packet.rows ) 
	 ~sql:sql)
    in
    ()
  in
  ...
  let () = 
    ...
    assert_equal ~msg:sql 
	 ...
	 [
	  [Mp_data.Time (Mp_data.Positive, 1, 1, 1, (Int64.of_int 999900)); 
	   Mp_data.Datetime ((2011, 11, 6), (1, 10, 2, (Int64.of_int 789510))); 
	   Mp_data.Date (0, 0, 0);
	   Mp_data.Time (Mp_data.Negative, 736, 41, 21, (Int64.of_int 990000));
	   Mp_data.Time (Mp_data.Negative, 0, 0, 51, (Int64.of_int 340000));
  ...
  let () = 
    ...
    let stmt = Mp_client.create_statement_from_string sql in
    let p = Mp_client.prepare ~connection:connection ~statement:stmt in
    let params = [List.nth (List.nth records 0) 0; Mp_data.String "...special ' characters \" ..."] in
    assert_equal ~msg:sql 
      ~cmp:records_equals
...
let test_date connection records_date version = 
  ...
  let () = 
	...
	[Mp_data.Varstring "13:17:05.000000"]
      else
	[Mp_data.Varstring "13:17:04.000000"]
    in
    assert_equal ~msg:sql 
      ~cmp:records_equals
      ( ([("f1", 0)], [[Mp_data.Varstring "00:00:00.000000"]; df2]) )
      (Test_query.try_query 
	 ~f:( (Mp_client.get_result_set(Mp_client.execute ~connection:connection ~statement:p ())).Mp_result_set_packet.rows ) 
	 ~sql:sql)
  in
...
let test_filter_iter connection records ok_value_iter = 
  let () = 
	...
	( (fields, [List.nth records 0]) )
	(Test_query.try_query 
	 ~f:( (Mp_client.get_result_set(Mp_client.execute ~connection:connection ~statement:p ~filter:(Some filter) ~iter:(Some (iter acc)) ())).Mp_result_set_packet.rows ) 
	 ~sql:sql)
    in
    assert_equal ~msg:sql 
      ok_value_iter !acc
  ...
  *)
  ...
  let records_geo = 
    if (version <= 5095) then (
      [ [Mp_data.Varbinary (Buffer.add_string blobgeo1 "POINT(231 4)"; blobgeo1); ];
        [Mp_data.Varbinary (Buffer.add_string blobgeo2 "POINT(-2 5)"; blobgeo2); ] ]
     )
    else (
      [ [Mp_data.Blob (Buffer.add_string blobgeo1 "POINT(231 4)"; blobgeo1); ];
        [Mp_data.Blob (Buffer.add_string blobgeo2 "POINT(-2 5)"; blobgeo2); ] ]
     ) 
  in
  let module F = (
    val (
