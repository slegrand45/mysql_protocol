let data_value_to_native_value field data = 
  match data with 
  | Mp_raw_data.Row_data_null -> Mp_data.Null
  | Mp_raw_data.Row_data_data v -> Mp_data_simple.data_value_to_native_value v field
  | Mp_raw_data.Row_data_binary v -> Mp_data_binary.data_value_to_native_value v field
;;
...
let data_row field_packets list_packets = 
  ...
  let one_packet acc data = 
    ...
    let v = data_value_to_native_value field data in
    v :: acc
  in
...
let to_bitstring_longlongint v = 
  ...
  with
  | Failure ex -> ( (* the big integer is > 2^63 - 1*)
