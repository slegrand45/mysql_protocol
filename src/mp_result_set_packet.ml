...
type result_select = { 
    rows : (Mp_field.field_name list * Mp_data.native_data list list);
...
let result_set_packet packet_length result_set_field_count bits ic oc filter iter return_all_raw_mysql_data type_sent fields =
  ...
  let () =
	...
	if (not !data_packets_end) then
	  ...
	  in
	  let native_data = Mp_data_process.data_row list_field_packets l in
	  let filtered = ref false in
	  let () = 
	    match filter with 
	    | None -> list_native_data := native_data :: !list_native_data
	    | Some f ->
		if (f list_field_names native_data) then 
		  list_native_data := native_data :: !list_native_data
		else
		  filtered := true
	  in
	  let () = 
		...
		if (not !filtered) then
		  f list_field_names native_data
	  in
  ...
  let list_native_data = List.rev !list_native_data in
