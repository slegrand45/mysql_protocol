...
let data_equals d1 d2 = 
  let f acc e1 e2 = 
    ...
    let () = 
      match e1 with
      | Mp_data.Blob _ | Mp_data.Binary _ | Mp_data.Varbinary _ -> (
	  if (not comparison) then (
...
let test1 connection records db_name version = 
  let () = 
	...
	(Test_query.try_query 
	   ~f:( (Mp_client.get_result_set(Mp_client.execute ~connection:connection ~statement:stmt ())).Mp_result_set_packet.rows ) 
	   ~sql:sql)
    in
    let f = 
      let r = Mp_client.get_result_set(Mp_client.execute ~connection:connection ~statement:stmt ~return_all_raw_mysql_data:true ()) in
...
let test_date connection records_date version = 
  ...
  let () = 
    ...
    assert_equal ~msg:sql 
	 ...
	 [
	  [Mp_data.Time (Mp_data.Positive, 1, 1, 1, (Int64.of_int v9999)); 
	   Mp_data.Datetime ((2011, 11, 6), (1, 10, 2, (Int64.of_int v78951))); 
